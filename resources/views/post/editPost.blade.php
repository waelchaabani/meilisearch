...
<div>
        <div class="w-full px-16 py-20 mt-6 overflow-hidden bg-white rounded-lg lg:max-w-4xl">
          <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
            <form method="post" action="{{ route('posts.update',$editData->id) }}">
              <!-- Title -->
              @csrf
              ...
              <div>
                ...
                <input wire:model="title" value="{{ $editData->title }}" 
              ...
              <div class="mt-4">
               ...
               <input wire:model="body" value="{{ $editData->body }}" 
                  class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 placeholder:text-right focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                  type="text" name="body" required="" placeholder="Enter Title"/>
                  ...
                  <select class="form-control select2" wire:model="category_id" data-placeholder="Choose country" name="category_id" required="">
                    ...
                    <option value="{{ $category->id }}" {{ ($editData->category_id  ? "selected": "") }}>  
                    {{$category->name}} 
</option>
              ...
              <x-jet-button class="px-8 float-right mr-2" type="submit" value="Update"  >Update Post</x-jet-button>
